  ...
  return (
    <footer className="bg-gray w-screen pt-10 md:pt-20 pb-5 text-white px-10 md:px-16 flex flex-col gap-y-16">
        ...
        <section className='flex flex-col items-center space-y-5 text-center'>
            ...
            <form onSubmit={submitSubscription} className='flex flex-col gap-y-5 w-full max-w-[300px]'>
                <input 
                    type="email" 
                ...
                <button type='submit' className='uppercase bg-white hover:bg-transparent hover:border-2 text-black py-2 hover:text-white transition rounded'>Subscribe</button>
            </form>

            {/* Display Success Message */}
            <div className={`${success ? 'block animate-slideY': 'hidden'} space-y-3 shadow-md p-5 bg-white text-black text-start rounded-md`}>
                <h2 className='font-semibold text-2xl'>Thank you!.</h2> 
                <p className=''>Your subscription has been confirmed. You've been added to our list and you'll hear from us soon</p>
            </div>
        </section>
